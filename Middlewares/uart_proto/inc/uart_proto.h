...
#define CUSTOM_UART_PROTO_CONFIG        1  /**< Enable custom UART protocol config */
 ...
 *  @{
 */
#define NUM_NOTIFY_ISR_CB_CALL          3           /**< ISR callback retries */
#define MAX_PARSE_NUM_ONCE_TRIGGER      10          /**< Max frames parsed per ISR callback */
#define PARSE_THREAD_PRIORITY           16          /**< Parsing thread priority */
#define PARSE_THREAD_STACK_DEPTH        0x200       /**< Parsing thread stack size */
...
/* -------------------------------------------------------------------------- */
#include "Debug.h"
#ifdef DEBUG_UART_PROTO
#define UP_DEBUG_OUT(fmt, ...)      DEBUG_OUT(fmt, ##__VA_ARGS__)
#define UP_DEBUG_ERR(fmt, ...)      DEBUG_OUT_ERR(fmt, ##__VA_ARGS__)
#else
#define UP_DEBUG_OUT(fmt, ...)    
#define UP_DEBUG_ERR(fmt, ...)
#endif
...
typedef struct
{
    ...
    uint16_t pre_payload_len;  /**< Header length in bytes */
    uint16_t post_payload_len; /**< Trailer length in bytes */
 ...
 */
typedef void (*pf_fun_code_cb_t)(void *arg, uint8_t *const payload, uint16_t payload_len);
...
#endif
#if (UART_PROTO_MODE_DEFAULT == UART_PROTO_MODE_DUAL_STRATEGY)
    uart_proto_status_t (*pf_strategy_algo)(struct uart_proto *const self,
