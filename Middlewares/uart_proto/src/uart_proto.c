                                             ...
                                             void **handle)
{
    ...
    while (cur->next != head)
    {
        funcode_node_t *next = T_LIST_ENTRY(cur->next, funcode_node_t, list);
        if (next->fun_code > para->fun_code)
            break;
        cur = cur->next;
    }
    t_list_insert_after(cur, &fn->list);
 ...
 */
static uart_proto_status_t strategy_algo(uart_proto_t *const self,
                                         parse_algo_t *algo)
{
    if (!self || !algo)
        return UART_PROTO_ERR_PARAM_INVALID;
                                         ...
                                         uint16_t len)
{
    ...
    while ((status = FUNCODE_ALGO(self)(addr + used, len - used, &info)) == ALGO_OK ||
           ...
           status == ALGO_ERR_NOICE)
    {
        ...
        parse_info_t pi = {
            ...
            .payload = addr + used + info.pre_payload_len,
            .payload_len = info.payload_len
        };

        /* Update parsed length including header/trailer */
        used += info.pre_payload_len + info.payload_len + info.post_payload_len;
        UP_DEBUG_OUT("unpack_len=%d\r\n", used);
                                       ...
                                       parse_info_t *info)
{
    ...
    for (t_list_t *cur = head->next; cur != head; cur = cur->next)
    {
        funcode_node_t *node = T_LIST_ENTRY(cur, funcode_node_t, list);

        if (node->fun_code > info->fun_code)
            break;

        if (node->fun_code == info->fun_code && node->cb)
            node->cb(node->arg, info->payload, info->payload_len);
    }
}
#endif
...
static uint16_t parse_transparent_mode(uart_proto_t *const self,
                                       ...
                                       uint16_t len)
{
    parse_info_t info = { .payload = addr, .payload_len = len };
...
static void handle_transparent_parse(uart_proto_t *const self,
                                     parse_info_t *pi)
{
    TRANS_ALGO(self)(pi->payload, pi->payload_len, TRANS_ARG(self));
}
#endif
...
static void parse_thread(void *arg)
{
    ...
    while (1)
    {
        ...
        UP_DEBUG_OUT("Frame parsed, handling...\r\n");
                                    ...
                                    uart_proto_input_arg_t *const args)
{
    ...
    uint32_t stack = PARSE_THREAD_STACK_DEPTH;
    uint32_t prio = PARSE_THREAD_PRIORITY;
#if (CUSTOM_RX_THREAD_ATT)
    if (THREAD_ATT(self))
    {
        stack = PARSE_THREAD_ATT(self).stack_depth;
        prio = PARSE_THREAD_ATT(self).thread_priority;
    }
#endif
...
#endif
#if (UART_PROTO_MODE_DEFAULT == UART_PROTO_MODE_DUAL_STRATEGY)
    self->pf_strategy_algo = strategy_algo;
#endif

    PRIV_DATA(self)->is_inited = true;
    UP_DEBUG_OUT("uart proto instance init success!\r\n");
    return UART_PROTO_OK;
}
...
{
    PRIV_DATA(self)->parse_fail_cnt = PRIV_DATA(self)->header =
...
void notify_isr_cb(uart_proto_t *const self)
{
    ...
    uint16_t pre = PRIV_DATA(self)->tail % RECV_BUF_SIZE(self);
    uint16_t end = RECV_BUF_SIZE(self) - UART_INTERFACE(self)->pf_get_counter();
    uint16_t len = (end - pre + RECV_BUF_SIZE(self)) % RECV_BUF_SIZE(self);

    /* Update header counter to detect overflow */
    PRIV_DATA(self)->header += (end - PRIV_DATA(self)->data_counter + RECV_BUF_SIZE(self)) %
                          RECV_BUF_SIZE(self);
    ...
    {
        UP_DEBUG_ERR("Buffer overflow detected.\r\n");
        reset_rx_state(self);
        OS_INTERFACE(self)->pf_os_exit_critical(primask);
        UP_TRACE_ISR_EXTI();
        return;
    ...
    PRIV_DATA(self)->data_counter = end;
    ...
    if (len > 0)
    {
        ...
        else
        {
            ...
            uint16_t len1 = RECV_BUF_SIZE(self) - pre;
            uint16_t len2 = end;
            memcpy(PRIV_DATA(self)->parse_buf, RECV_BUF(self) + pre, len1);
            memcpy(PRIV_DATA(self)->parse_buf + len1, RECV_BUF(self), len2);
#ifdef NON_COPY_WHEN_NON_WRAP
            wrapped = true;
#endif
        }
...
#ifdef NON_COPY_WHEN_NON_WRAP
    uint8_t *parse_addr = wrapped ? PRIV_DATA(self)->parse_buf : (RECV_BUF(self) + pre);
#else
    ...
    uint16_t used_len = 0;
#if (UART_PROTO_MODE_DEFAULT == UART_PROTO_MODE_FUNCTION_CODE)
    used_len = parse_function_code_mode(self, parse_addr, len);
#elif (UART_PROTO_MODE_DEFAULT == UART_PROTO_MODE_TRANSPARENT)
    used_len = parse_transparent_mode(self, parse_addr, len);
#elif (UART_PROTO_MODE_DEFAULT == UART_PROTO_MODE_DUAL_STRATEGY)
    if (ALGO_FUNCODE == ALGO_TYPE(self))
        used_len = parse_function_code_mode(self, parse_addr, len);
    else
    ...
    PRIV_DATA(self)->tail += used_len;

    /* --- Error handling --- */
    if (PRIV_DATA(self)->parse_fail_cnt >= PRIV_DATA(self)->num_notify_isr_cb_call)
    {
        UP_DEBUG_ERR("Parse failure threshold exceeded, reset.\r\n");
        reset_rx_state(self);
    }
